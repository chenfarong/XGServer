...
#       define XASSERT( x )           if ( !((void)0,(x))) { __debugbreak(); }
#   elif defined (ANDROID_NDK)
#       include <android/log.h>
#       define XASSERT( x )           if ( !(x)) { __android_log_assert( "assert", "grinliz", "ASSERT in '%s' at %d.", __FILE__, __LINE__ ); }
#   else
#       include <assert.h>
...
#define FALSE 0
	...
	static int IsTextUTF8(const char* str, ULONGLONG length)
	{
		int i;
	...
	static size_t utf8length(std::string& utf8str)
	{
		try
		{
 	...
 	static bool WStrToUtf8(std::wstring wstr, std::string& utf8str)
	{
		try
		{
			std::string utf8str2;
	...
	static const char* ReadBOM(const char* p, bool* bom)
	{
		XASSERT(p);
		XASSERT(bom);
		*bom = false;
